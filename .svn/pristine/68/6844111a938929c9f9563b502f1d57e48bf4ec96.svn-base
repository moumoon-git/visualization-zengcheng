<template>
  <div class="troopslist_Wrap" >

    <div class="listcontent" v-for="(item,index) in data" :key='index'  @click="clickSelect(item,index)" >
      <img class="iconImg" v-show="!item.flag" src="../../../../../../assets/images/cszh/troops.png" alt="">
      <img class="iconImg" v-show="item.flag" src="../../../../../../assets/images/cszh/troops1.png" alt="">
      <div class="content" >
        <h3>{{item.name}}</h3>
        <p>{{item.address}}</p>
        <p>{{item.peopleNumber}}人  |  成立于{{item.creationTime}}</p>
        <p>负责人：
      ...
      <img class="listcontentImg" src="" alt="">
    </div>
  ...
  export default{
    ...
    methods:{
      clickSelect(item,index) {
        ...
        item.flag = true
        this.$emit('clickSelects',{data:item,ind:index,type:'dw'})
      },
      init (arrData) {
        arrData.forEach(ele=>{
          ele.flag = false
