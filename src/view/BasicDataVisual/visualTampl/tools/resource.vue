  ...
  methods: {
    ...
    checked(item, index) {
      ...
      if (index == 0) {
        ...
        this.xfz(item);
      } else if (index == 4 || index == 5 || index == 6 || index == 7 || index == 8 || index == 9 || index == 10 || index == 11) {
        this.spjk(item);
      }
    },
    ...
    render(type) {
      ...
      if (type == 0) {
        ...
        this.mackerData[15] = this.forEachData(this.data[15], wgsjIcon, 15);
      } else if (type == 16) {
        this.mackerData[16] = this.forEachData(this.data[16], xfzIcon, 16);
      } else if (type == 5 || type == 6 || type == 7 || type == 8 || type == 9 || type == 10 || type == 11) {
    ...
    forEachData(data, imgUrl, num) {
      console.log("forEachData data, imgUrl, num", data, imgUrl, num);
      let that = this;
      // let requestApi = "",
      //   _data = data;
      ...
      if (num == 1) {
        ...
        if (data.length != 0) {
          ...
          window.map.setCenter([templateElement.longitude||templateElement.lon, templateElement.latitude||templateElement.lat]);
        ...
        window.map.setCenter([data[0].longitude||data[0].lon, data[0].latitude||data[0].lat]);
        return mass;
      } else {
        // 创建一个 icon
        var endMarkerList = [];
        ...
        if (data.length != 0) {
          ...
          this.map.add(endMarkerList);
          console.log("templateElement.longitude,templateElement.latitude",templateElement.longitude,templateElement.latitude)
          window.map.setCenter([templateElement.longitude||templateElement.lon, templateElement.latitude||templateElement.lat]);
          return endMarkerList;
        }
      } 
...
.resource_ul > li:nth-child(n + 6):nth-child(-n + 12).monitorExpand {
  display: block;
}
