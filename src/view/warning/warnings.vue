  ...
  <div class="warnings_wrap">
    ...
    <div class="warnings_right">
      <div
        ...
        "
      >
        <div class="blueTitle" style="margin-bottom: 20px;position: relative;">实时预警信息
        <div class="warnings_right_search">
            <el-input
              :placeholder="$store.state.selectBtn == 'human' ? '请输入人员名称、身份证号码' : '请输入车牌号码' "
              suffix-icon="el-icon-search"
              clearable
              @change="$store.state.selectBtn == 'human' ? getHumanLists('search') : getriskvehiclealarm('search')"
              @clear="$store.state.selectBtn == 'human' ? getHumanLists('clear') : getriskvehiclealarm('clear')"
              v-model="inputValue">
            </el-input>
        </div>
        </div>
      ...
      <div class="historicalInformation">
        ...
        >
          ...
          <div class="border_wrap historicalInformation_content_div">
            <p v-if="$store.state.selectBtn == 'human'" >{{ humanEntity.listLibName }}</p>
            <p v-if="$store.state.selectBtn != 'human'" >{{ carType }}</p>
  ...
  methods: {
    ...
    setHotMap(data) {
      ...
      that.allFlag.rlFlag = true;
          console.log('that.humanjsMap: ', that.humanjsMap);
      if (that.humanjsMap) {
    ...
    getTodayRecordAndDraw(id) {
      ...
      let  setMapCenter = ()=>{
        ...
        if( (res.page.records[0].longitude && res.page.records[0].latitude) || (res.page.records[0].riskCameraEntity.longitude && res.page.records[0].riskCameraEntity.latitude) ) {
          longitude = res.page.records[0].longitude || res.page.records[0].riskCameraEntity.longitude;
          latitude = res.page.records[0].latitude || res.page.records[0].riskCameraEntity.latitude;
          this.map.setZoomAndCenter(20, [longitude, latitude]);
          let img = require("../../assets/images/camera.png");
          this.addMarck(longitude, latitude, img);
        }
      ...
      let data = allUrl.getHumanList,
        s = {
          params: {
            ...
            limit: 20,
            search:type == 'clear' ? '' : this.inputValue
          }
        };
      Object.assign(data, s); // 合并对象, 只是把参数搞成对象形式 {}
      this.apix(data)
      ...
      this.apix(data)
        .then(res => {
          ...
          if (type == 1) {
            ...
            this.$refs.rankingListx.init(res.human,this.map);
          } else if (type == 2) {
            ...
            this.$refs.rankingListx.init(res.car,this.map);
          }
        })
        .catch(err => {
          console.log(err);
      ...
      this.apix(data)
        .then(res => {
          ...
          if (this.$refs.HeaderInformation && this.$refs.rankingListxTow) {
            ...
            this.setHotMap(res.data);
            this.$refs.rankingListxTow.init(res.data,this.map);
          }
        })
        .catch(err => {
          console.log(err);
    ...
    getHotIndexFive(type) {
      ...
      this.apix(data)
        .then(res => {
          console.log('/statisticApi/humanAlarmApi/getAlarmHumanCountGroupByTypeTop5 res', res);
          ...
          console.log("this.fragmentData,this.humanAndCarAllData",this.fragmentData,this.humanAndCarAllData)
        })
        .catch(err => {
          console.log(err);
        });
      ...
      this.apix(data)
        .then(res => {
          ...
          if (this.humanFiveTopSelectPage == 1) {
            ...
            this.track(arrAy);
            setTimeout(() => {
              _that.$refs.trackCamera.init(res.page.records, res.camera,this.map);
            });
          } else {
            setTimeout(() => {
      ...
      let data = allUrl.riskvehiclealarm,
        s = {
          params: {
            ...
            platformId: this.$store.state.platformId,
            token: this.$store.state.token,
            carNum:type == 'clear' ? '' : this.inputValue
          }
        };
      Object.assign(data, s); // 合并对象, 只是把参数搞成对象形式 {}
      this.apix(data)
        .then(res => {
          res.page.records.length == 0 ? this.noData = true : this.noData = false
    ...
    carFiveTopSelect(item) {
      ...
      this.apix(data)
        .then(res => {
          this.carType = res.blacklistEntity.carType;
          if (this.carFiveTopSelectPage == 1) {
            ...
            setTimeout(() => {
              _that.$refs.trackCamera.init(res.page.records, res.cameras,this.map);
            });
            this.allPhotograph = res.page.total;
            this.allFlag.warnings_rightFlag = false;
            this.allFlag.warnings_rightTowFlag = true;
...
/deep/ .el-input__inner {
    ...
    border: 1px solid #0841c7 !important;
  }
}
</style> 
