<template>
  <div class="MappingTools_wrap" >
      ...
      <div v-show="resourceFlag" class="resourceBig_wrap animated fadeIn"   >
        <resource ref='resource' v-on="$listeners" @PaintingPanelHandle="PaintingPanelHandle"></resource>
      </div>
      <!-- 路径分析 -->
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      PaintingPanelHandle(item){
          this.$refs.PaintingPanel.gridDistribution(item)
      }
    },
    components:{
      serch,
    ...
    mounted () {
      bus.$on("getGridEventsDetail",(id)=>{
        this.clickTools({name:'图层工具',flag:false},0)
        console.log('this.$refs.PaintingPanel: ', this.$refs.PaintingPanel);
        this.$refs.PaintingPanel.gridDistributionDialog(id)
      })
    },
    watch:{
