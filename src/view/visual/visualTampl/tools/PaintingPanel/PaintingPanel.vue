...
export default {
  ...
  mounted() {
    ...
    bus.$on("getOverlays",()=>{
        this.$emit('getOverlayers',this.overlays);
    })
  ...
  methods: {
    ...
    initContextMenu() {
        ...
        () => {
          this.$refs.PaintingPanelOverlayEditDialog
            .edit(
              ...
              this.rightClickObjText.getExtData().gridName,
              this.rightClickObjText.getExtData().gridArea,
              this.rightClickObjText.getExtData().gridNumber,
              this.rightClickObjText.getExtData().jurisdictionalScope,
              this.rightClickObjText.getExtData().basicInformation,
              this.rightClickObjText.getExtData().operatingDuty,
              this.rightClickObjText.getExtData().remark,
              this.rightClickObjText.getExtData().personCharge,
            )
        },
        0
      )
      ...
      contextMenu.addItem(
        '删除',
        () => {
          this.rightClickObj.hide()
          this.rightClickObjText.hide()
        ...
        //   this.overlays.splice(this.overlays.findIndex(e => Object.is(e.obj, this.rightClickObj)), 1)
          let currentData = this.rightClickObjText.getExtData()
          currentData.isDelete = true
          this.rightClickObjText.setExtData(currentData)
          bus.$emit("closeDistribution",this.rightClickObjText.getExtData().id)
        },
        1
      )
    ...
    draw({
      ...
      mouseTool.on('draw', ({ obj }) => {
        ...
        // 5.2 mouseTool所绘制的矩形实际上是多边形，无法应用矩形编辑器，需要转换成矩形
        if (type === 'rectangle') {
        ...
        let path = obj.getPath().map(e => [e.getLng(),e.getLat()])
        if (type === 'circle'){
            gridObj.gridArea = (3.14*obj.getRadius()*obj.getRadius()).toFixed(2) + "平方米"
        }else if(type === 'rectangle' || type === "polygon"){
            gridObj.gridArea = Math.round(AMap.GeometryUtil.ringArea(path)).toFixed(2) + "平方米"
        }   
    ...
    bindEvent(obj, text, type) {
      ...
      const rightclickCallback = event => {
        if (this.isActive) {
          ...
          console.log('text: ', text.getExtData());
    ...
    update({
      ...
      let gridObj= {
        ...
        remark,
        personCharge,
        id:this.rightClickObjText.getExtData().id,
      }
      this.rightClickObjText.setText(text)
      this.rightClickObjText.setTitle(title)
      this.rightClickObjText.setExtData(gridObj)
    ...
    save() {
      ...
      if (!this.saveLimit) {
        ...
        this.overlays.forEach(overlay => {
          if (!overlay.text.getExtData().isDelete) {
              ...
              let item = {
                ...
                let submitData = overlay.text.getExtData()
              let area = submitData.gridArea
              var gridObj= {
                  id:submitData.id,
                  gridName: submitData.gridName,
                  gridArea:  area?parseFloat((area+"").substr(0,area.length-3)):"",
                  gridNumber: submitData.gridNumber,
                  areaRange: submitData.jurisdictionalScope,
                  basicSituation: submitData.basicInformation,
                  jobResponsibility: submitData.operatingDuty,
                  gridRemark: submitData.remark,
                  principals: (submitData.personCharge&&submitData.personCharge.length>0)?submitData.personCharge.map(v=>v.id):[],
              }
              if (overlay.obj.CLASS_NAME == 'AMap.Circle' || overlay.obj.CLASS_NAME ==  'AMap.Rectangle' || overlay.obj.CLASS_NAME == 'AMap.Polygon') {
                  item = {
                      ...item,
                      ...gridObj
                  }
        ...
        this.apix(request)
          .then(response => {
            if (response.code === 0) {
              ...
              this.beforeGrid = this.overlays.filter(v=>!v.text.getExtData().isDelete)
              this.getData()
    ...
    reDraw(record) {
      ...
      record.forEach(data => {
        ...
        let gridObj = {
            ...
            gridArea:data.gridArea?data.gridArea.toFixed(2)+"平方米":"",
            gridNumber:data.gridNumber,
            jurisdictionalScope:data.areaRange,
            basicInformation:data.basicSituation,
            operatingDuty:data.jobResponsibility,
            remark:data.gridRemark,
            personCharge:data.principal,
            id:data.id,
        }
        ...
        if (data.type == 'polygon' || data.type ==  'circle' || data.type == 'rectangle') {
            if (this.beforeGrid.length == 0) {
                reDrawOverlay.hide()
                reDrawText.hide()
            }else{
                this.beforeGrid.map(v=>{
                    if (v.text.getExtData().id == data.id) {
                        if (v.obj.getVisible()) {
                            reDrawOverlay.show()
                            reDrawText.show()
                        }else{
      ...
      if (this.$parent.$parent.integratedGridFlag) {
          bus.$emit("updateWorkList",this.overlays)
      }
    },
    /**
    ...
    clear(type) {
      ...
      if (type === false) {
        this.overlays.forEach(overlay => {
            overlay.obj.hide()
            overlay.text.hide()
            let currentData = overlay.text.getExtData()
            currentData.isDelete = true
            overlay.text.setExtData(currentData)
        })
      }else{
          this.overlays.forEach(overlay => {
    ...
    // 点击综合保障是否选中判断网格分布是否显示
    gridDistribution(item){
        this.overlays.map(v=>{
            if (v.obj.CLASS_NAME == 'AMap.Circle' || v.obj.CLASS_NAME ==  'AMap.Rectangle' || v.obj.CLASS_NAME == 'AMap.Polygon') {
                if(item.flag){
                    v.obj.show()
                    v.text.show()
                }else{
    ...
    gridDistributionDialog(id){
        let gridData = this.overlays.find(v=>v.text.getExtData().id == id)
        this.$refs.PaintingPanelOverlayEditDialog
            .edit(
              gridData.text.getText(),
              gridData.text.getTitle(),
              gridData.obj.getOptions().fillColor,
              gridData.obj.getOptions().strokeWeight,
              gridData.obj.getOptions().fillOpacity,
              gridData.obj.CLASS_NAME,
              gridData.obj.getOptions().strokeColor,
              gridData.text.getExtData().gridName,
              gridData.text.getExtData().gridArea,
              gridData.text.getExtData().gridNumber,
              gridData.text.getExtData().jurisdictionalScope,
              gridData.text.getExtData().basicInformation,
              gridData.text.getExtData().operatingDuty,
              gridData.text.getExtData().remark,
              gridData.text.getExtData().personCharge,
            )
        this.rightClickObjText=gridData.text
        this.rightClickObj=gridData.obj
    },
  },
  destroyed(){
    bus.$off("gridDistribution")
  }
}
</script>
