<template>
  <el-dialog class="link-man" :title="title" :visible.sync="visible" width="1100px" center append-to-body :close-on-click-modal="false" :modal="false">
    ...
    <el-row :gutter="20">
      <el-col :span="8">
        <!-- <input v-model="search" placeholder="输入姓名、单位名搜索" @keyup.enter="onSearch">
      ...
      <el-col :span="8">
        <!-- <el-input v-model="search" placeholder="输入姓名、单位名搜索" @keyup.enter="onSearch">
...
export default {
  ...
  methods: {
    ...
    // 点击添加联系人事件
    handleAddContacts (url, title, name,checkContactors) {
      if (checkContactors) {
          this.nameList = checkContactors.map(v=>v.id)
          this.chooseList = [...checkContactors]
      }
      let self = this
...
.link-man-department >>> .el-tree-node__content:hover {
  background-color: #0091FF !important;
}

.candidate >>> .el-pager > li {
