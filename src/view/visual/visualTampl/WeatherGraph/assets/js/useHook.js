export default () => {
  // 获取月日 星期
  const getDate = () => {
    const month = zeroFilling(new Date().getMonth() + 1);
    const day = zeroFilling(new Date().getDate());
    const week = new Date().getDay();
    const weekStr = "日一二三四五六";
    return `${month}月${day}日 周${weekStr.charAt(week)}`;
  };

  // 补零
  const zeroFilling = num => {
  ...
  const getLunarCalendar = () => {
    ...
    function Draw() {
      ...
      let lunar_month_day = lmonth + "月" + lday;
      //需要展示在页面地方
    ...
    //辅助函数
    const hzWeek = new Array("日", "一", "二", "三", "四", "五", "六", "日");
    ...
    function getlunar() {
      let lObj = new Lunar(nyear, nmonth - 1, nday);
